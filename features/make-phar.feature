Feature: Check `utils/make-phar.php` output
  ...
  Scenario: Check autoload stripping of phpcs development classes
    ...
    And a custom-cmd.php file:
      ...
      FIN_CLI::add_command( 'command example', 'Dealerdirect\Composer\Plugin\Installers\PHPCodeSniffer\Plugin' );
      """
  ...
  Scenario: Phar renaming affects template path resolution
    ...
    When I run `cp {PHAR_PATH} fin`
    And I try `php fin plugin status generic-example-plugin`
